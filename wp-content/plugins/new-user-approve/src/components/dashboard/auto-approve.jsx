...
const Auto_Approve = () => {
  ...
  const [isWhitelistEnabled, setWhitelistToggle] = useState(false);
  const [isBlacklistEnabled, setBlacklistToggle] = useState(false);
  ...
  const currentTab = location.pathname.split('/')[2] || 'tab=whitelist';

  useEffect(() => {
    if (currentTab === 'tab=whitelist') {
      navigate('tab=whitelist');
    }
    // Don't show popup on mount
  }, []);
  ...
  const handleChange = (event) => {
    ...
    // setPopupVisible(true); // Show popup only when user types
  };
  ...
  return (
    ...
    <Tabs className = "auto-approve-tabs nua-auto-settings"  defaultValue={'tab=whitelist'} value={currentTab} onChange={handleTabChange}>
      <TabsList className ="auto-approve-tab-list">
        <Tab value={'tab=whitelist'} style={{display: 'flex' , alignItems: 'center', gap: '6px'}} className={currentTab == 'tab=whitelist' ? 'nua_active_subtab' : '' }>{__('WhiteList', 'new-user-approve') }{proIcon}</Tab>
        <Tab value={'tab=blacklist'} style={{display: 'flex' , alignItems: 'center', gap: '6px'}} className={currentTab == 'tab=blacklist' ? 'nua_active_subtab' : '' }>{__('BlackList', 'new-user-approve') }{proIcon}</Tab>
      </TabsList>

      <TabPanel className={currentTab == 'tab=whitelist' ? 'auto-approve-whitelist': ''} value={'tab=whitelist'} >
           
      <div className='enable-whitelist domain-list setting-option nua-setting-pro'>
    
        <span style={{fontSize: '1.3em'}} className='setting-label enable-whitelist-label'>
          {__('Enable Whitelist', 'new-user-approve')}
          <span className="nua-tooltip-wrapper">
            ...
            <div className="nua-tooltip-text">
              {__('Domains added in the Whitelist will be automatically approved.', 'new-user-approve')}
            </div>
          </span>
        </span>

        <div className='enable-whitelist-element setting-element'>
          <label className="nua_switch" htmlFor="isWhitelistEnabled">
            <input id="isWhitelistEnabled" name="isWhitelistEnabled" type="checkbox" checked={isWhitelistEnabled} onChange={handleToggleChange} />
        ...
        name="whitelist"
        id="whitelist"
        className={'whitelist-area nua-setting-textarea'}
        value={whitelist}
      ...
      <TabPanel className='auto-approve-blacklist'  value={'tab=blacklist'}>
    
      <div className='enable-blacklist domain-list setting-option nua-setting-pro'>
          <span className='setting-label enable-blacklist-label'>
            {__('Enable Blacklist', 'new-user-approve')}
            <span className="nua-tooltip-wrapper">
              <span className="dashicons dashicons-editor-help nua-tooltip-icon"></span>
              <div className="nua-tooltip-text">
                {__('Domains added in the Blacklist will be completely blocked and users using blacklisted domains will not be able to submit approval requests.', 'new-user-approve')}
              </div>
            </span>
  
          </span>

          <div className='enable-blacklist-element setting-element nua-setting-pro'>
            <label className="nua_switch" htmlFor="isBlacklistEnabled">
              <input id="isBlacklistEnabled" name="isBlacklistEnabled" type="checkbox" checked={isBlacklistEnabled} onChange={handleToggleChange} />
              <span className="nua_slider round"></span>
            </label>
          </div>
      </div>
      <div className="nua-domain-text nua-setting-pro">
       <div className="nua-domain-text-a">
        <span>{__(`Enter Domain(s)`, 'new-user-approve')}</span>
        
        <p className={'blacklist-desc auto-approve-textarea-desc'}>
          {__(`One domain per line.`, 'new-user-approve')}
        </p>
        </div>
        <div>
        ...
        name="blacklist"
        id="blacklist"
        className={'blacklist-area nua-setting-textarea'}
        value={blacklist}
        onFocus={setPopupVisible}
        onChange={handleChange}  />
        </div>
      ...
      <div className="nua-domain-text custom-message nua-setting-pro">
      <div className="nua-domain-text-a">
      <span>{__(`Custom Message`, 'new-user-approve')}</span>
      </div>
      <div className="nua-custom-message-b nua-setting-pro">
        <input 
        type="text"
        name="customMessage" 
